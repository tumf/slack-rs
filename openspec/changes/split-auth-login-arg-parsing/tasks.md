...
- [ ] 標準ログインと拡張ログインで重複する OAuth 実行手順を単一コア関数へ統合する（検証: 両経路が同一コア呼び出しを利用するコード経路を確認）。
- [ ] OAuth 通信部分は mock/stub で検証可能な単位に分け、外部クレデンシャル不要でテスト可能にする（検証: fixture ベーステストが `cargo test --lib` で成功）。
- [ ] 既存ログインコマンド互換を確認する（検証: `cargo test --test auth_integration` と `cargo test --test oauth_integration` と `cargo test --test manifest_generation_integration` が成功する）。
