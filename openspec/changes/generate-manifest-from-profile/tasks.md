1. OAuth 設定のデータ構造を `bot_scopes`/`user_scopes` に分割し、旧 `scopes` を読み込む後方互換を追加する。
   - 検証: `profiles.json` の旧形式を読み込むテストで `bot_scopes` に移譲されることを確認する。

2. `auth login` で bot/user スコープを対話的に入力できるようにする（デフォルト入力値は両方とも `all`）。
...
3. OAuth 認可 URL に `user_scope` を追加し、`bot_scopes`/`user_scopes` を分離して送信する。
